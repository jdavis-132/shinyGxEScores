...
(
  function(input, output, session)
  {
    output$introNote <- renderText({ "Determine which empirical GxE interactiopsn are likely to impact selection decisions depending on the environment(s) used for selection. If you use this tool, please cite Davis et al. (2025): https://www.biorxiv.org/content/early/2025/01/24/2025.01.21.634104. 
     Performance and genotype-by-environment interactions of one hybrid pair across three environment pairs. Box plots show population-level yield in the given environment (n = 163 -- 175 plots). Boxes indicate the range from the 25th -- 75th percentile of values. Black lines within the boxplots indicate the median value. Whiskers indicate the most extreme values within 1.5 times the interquartile range and black points indicate the values of data points outside that range. The colored lines connect the mean yields of each hybrid between environments. Asterisks denote a significant difference in mean yields between the two hybrids in the environment at a significance level of alpha = 0.05 according to Tukey’s HSD. The left panel shows a pair of environments where the hybrid pair did not change rank order and received an interaction importance score of 0. In the middle panel, the hybrids changed rank order in this pair of environments and had a significant difference in mean yield in one of the two environments, and they received an interaction importance score of 1. The right panel shows an environment pair where the hybrids changed rank order and had significant differences in mean yield in both environments, and they received an interaction importance score of 2.
     The generated figure below is an incidence matrix indicating the frequency with which two hybrids exhibited an interaction for yield between two environments that represents a potentially important change in the selection decision between environments. For each hybrid pair, interaction scores were summed across all environment pairs and divided by the total score possible for the hybrid pair based on the number of environments in which both hybrids were present. Hybrids are ranked in order of ascending yield BLUE values fitting the environment as a fixed effect." })
